...
const InfoPanel = ({ poi, onClose, crabPeopleImages, onCrabClick, onPlayAudio, onStopAudio, onOpenExternalUrl, sparrowShot, onJesusClick }) => {
  ...
  const getDescription = () => {
    if (poi.id === 'sparrowprince' && sparrowShot) {
      return 'The Sparrow Prince. Was a mysterious figure in South Park.'
    }
  ...
  const handleImageClick = (imagePath, index) => {
    if (poi.externalUrl && onOpenExternalUrl) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        backgroundColor: poi.id === 'casabonita' ? '#FF69B4' : '#FFD700'
      }}>
        <h2 style={{
          margin: 0,
          fontSize: '1.5rem',
            ...
            {images.map((img, idx) => {
              ...
              <div
                ...
                style={{
                  ...
                  transition: hasInteraction ? 'transform 0.2s, box-shadow 0.2s' : 'none',
                  overflow: 'hidden'
                }}
                onMouseOver={(e) => {
                  if (hasInteraction) {
